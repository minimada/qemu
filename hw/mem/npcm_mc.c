...
static uint64_t npcm8xx_mc_read(void *opaque, hwaddr addr, unsigned int size)
{
    if (addr == 0) {
        return 0x101;
    }
...
static void npcm8xx_mc_write(void *opaque, hwaddr addr, uint64_t v,
                             unsigned int size)
{
    qemu_log_mask(LOG_UNIMP, "%s: mostly unimplemented\n", __func__);
}

static const MemoryRegionOps npcm8xx_mc_ops = {
    .read = npcm8xx_mc_read,
    .write = npcm8xx_mc_write,
    .endianness = DEVICE_LITTLE_ENDIAN,
    .valid = {
        .min_access_size = 4,
        .max_access_size = 4,
...
static void npcm8xx_mc_realize(DeviceState *dev, Error **errp)
{
    NPCMMCState *s = NPCM_MC(dev);

    memory_region_init_io(&s->mmio, OBJECT(s), &npcm8xx_mc_ops, s, "regs",
                          NPCM8XX_MC_REGS_SIZE);
    sysbus_init_mmio(SYS_BUS_DEVICE(s), &s->mmio);
}

static void npcm8xx_mc_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    dc->desc = "NPCM8xx Memory Controller stub";
    dc->realize = npcm8xx_mc_realize;
}

static const TypeInfo npcm_mc_info[] = {
    {
        .name = TYPE_NPCM_MC,
        .parent = TYPE_SYS_BUS_DEVICE,
        .instance_size = sizeof(NPCMMCState),
        .class_init = npcm7xx_mc_class_init,
        .class_size = sizeof(NPCMMCClass),
        .abstract   = true,
    },
    {
    ...
    {
        .name = TYPE_NPCM8XX_MC,
        .parent = TYPE_NPCM_MC,
        .class_init = npcm8xx_mc_class_init,
    },
};
DEFINE_TYPES(npcm_mc_info);
