...
static void npcm_pspi_update_irq(NPCM8XXPSPIState *s)
{
    int level = 0;
...
static uint16_t npcm_pspi_read_data(NPCM8XXPSPIState *s)
{
...
static void npcm_pspi_write_data(NPCM8XXPSPIState *s, uint16_t data)
{
...
{
    NPCM8XXPSPIState *s = NPCM8XX_PSPI(obj);
...
{
    NPCM8XXPSPIState *s = NPCM8XX_PSPI(dev);
...
static const VMStateDescription vmstate_npcm_pspi = {
    ...
    .fields = (const VMStateField[]) {
        VMSTATE_UINT16_ARRAY(regs, NPCM8XXPSPIState, NPCM8XX_PSPI_NR_REGS),
        VMSTATE_END_OF_LIST(),
    },
};
...
static const TypeInfo npcm_pspi_types[] = {
    {
        .name = TYPE_NPCM8XX_PSPI,
        .parent = TYPE_SYS_BUS_DEVICE,
        .instance_size = sizeof(NPCM8XXPSPIState),
